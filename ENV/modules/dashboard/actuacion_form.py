        ...
        if guardar:
            ...
            try:
                if supabase:
                    ...
                    data_api = {k: v for k, v in data.items() if k != "resultado"}
                    # Pydantic espera datetime; normalizamos a medianoche
                    data_api["fecha_accion"] = f"{fecha_accion.isoformat()}T00:00:00"
                    if is_edit:
                        api_actualizar(act["crm_actuacionid"], data_api)
                    else:
                        api_crear(data_api)
