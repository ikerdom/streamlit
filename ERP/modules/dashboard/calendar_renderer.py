...
def render_calendar(
    ...
    # -----------------------------------------
    # Mostrar panel de detalle si está activo
    # -----------------------------------------
    ...
    if act_det_id:
        ...
        if act_obj:
            render_actuacion_form(act_obj, supabase)
    ...
    with col_side:
        ...
        if not acts:
            ...
            por_estado = {}
            for a in acts:
                est = a.get("estado") or "Sin estado"
...
# ======================================================
# 🟩 Tarjeta de actuación individual en el calendario
# ======================================================
def _render_actuacion_card(a, clientes_map, supabase, day_index):

    color = COLOR_ESTADO.get(a.get("estado"), "#6b7280")
    ...
    st.markdown(
        ...
        <div style="
            ...
            <small>
                ...
                · {canal} · {estado} · Prioridad: {prioridad}
            </small>
        </div>
        """,
    ...
    if not bloque_horario and a.get("estado") != "Completada":
        with btn2:
            if st.button("✅ Completar", key=f"btn_comp_{a['crm_actuacionid']}"):
                try:
                    ...
                    st.success("Actuación marcada como completada.")
                    st.rerun()
